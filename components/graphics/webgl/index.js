...
export function Arm() {
  const { scene: arm1 } = useGLTF('/models/starting_position.glb')
  const { scene: arm2 } = useGLTF('/models/transition.glb')
  ...
  useScroll(({ scroll }) => {
    ...
    const _scale = mapRange(1, 5, progress, from.scale, to.scale)
    ...
    <>
      ...
      <Float floatIntensity={custom ? 0 : 1} rotationIntensity={custom ? 0 : 1}>
        ...
        >
          ...
          {type === 1 && <primitive object={arm1} scale={[2, 2, 2]} />}
          {type === 2 && <primitive object={arm2} scale={[2, 2, 2]} />}
          {/* </TransformControls> */}
        </group>
      </Float>
      {/* {target && (
