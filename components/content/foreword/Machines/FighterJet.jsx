...
export function Model(props) {
 const scroll = useScroll()
  const { nodes, materials, animations } = useGLTF("/assets/models/vehicles/pbr_fighter.glb");
    const { ref, actions } = useAnimations(animations)
  useEffect(() => void (actions["LandingGear_Up"].reset().play().paused = true), [])
  useFrame(() => (actions["LandingGear_Up"].time = actions["LandingGear_Up"].getClip().duration * scroll.offset))
  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Sketchfab_Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]}>
          <group
            name="FalconFighter_SFModelfbx"
...
useGLTF.preload("/assets/models/vehicles/pbr_fighter.glb");
