...
export const CapsModel = () => {
    ...
    const { nodes, materials } = useGLTF("/assets/models/apparel/Cap.glb");
    ...
    return (<>
      {capProps.map((p, idx) => {
            ...
            if (!currCapAnimation)
                return;
            return (<Scrollytelling.Animation key={idx} tween={{
    ...
    onReverseCall={() => {
      ...
      <group ref={innerRef}>
        {capProps.map(({ position, rotation }, idx) => {
            return (<group scale={responsiveVPWidth / 9} position={position.clone().multiplyScalar(halfViewportWidth)} rotation={rotation.clone()} key={idx}>
              <Float>
                <mesh geometry={nodes.Sphere007.geometry} material={clonedMaterials[idx]?.["m_Cap-v2"]}/>
                <mesh geometry={nodes.Sphere007_1.geometry} material={clonedMaterials[idx]?.["m_Outline"]}/>
              </Float>
            </group>);
        })}
      </group>
