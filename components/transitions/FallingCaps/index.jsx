...
const splitText = (text, wordClass) => {
    ...
    const htmlWords = wordsArray.map((word, i) => {
        ...
        return (<span className={wordClass} key={i}>
        {word}
...
const lines = ["Speed is divine", "Speed is eternal", "Speed is absolute."];
export const FallingCaps = () => {
    const splittedText = useMemo(() => lines.map((line, lineIdx) => {
        const isLast = lineIdx === lines.length - 1;
        const wordElements = splitText(line + "\n", isLast ? s["highlight"] : undefined);
        return wordElements;
    }).flat(), []);
    const perWordTimeline = useMemo(() => getTimeline({
        start: 0,
        end: 50,
        /* Chunk per word */
    ...
    return (<Scrollytelling.Root end="bottom bottom">
      <section className={s["spacer"]}>
        <div className={s["pin"]}>
          <div className={s["canvas-container"]}>
            <Canvas camera={{ position: [0, 0, 10], fov: 35 }} gl={{
            alpha: true,
            antialias: true,
          ...
          <p className={s["paragraph"]}>
            ...
            return (<Scrollytelling.Animation tween={{
